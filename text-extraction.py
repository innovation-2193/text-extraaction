    ...
    with zipfile.ZipFile(file_buffer, 'r') as docx_zip:
        ...
        for i, page_text in enumerate(pages, start=1):
            ...
            if page_header.strip():
                final_output += f"{page_header.strip()}\n\n"
                
            # แปะเนื้อหาหลักของหน้านี้
            if page_text.strip():
                final_output += f"{page_text.strip()}\n\n"
            
            # แปะ Footer ไว้ด้านล่างของหน้านี้
            if page_footer.strip():
                final_output += f"{page_footer.strip()}\n\n\n"
...
✅ ประกอบส่วนหัวและส่วนท้ายเข้าด้วยกันใน **ทุกๆ หน้า** อย่างต่อเนื่อง
